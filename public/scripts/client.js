...
const resetErrorMessage = violation => {
  if (violation === 'over count') {
    $(".error-message").hide();
    $(".error-message").empty();
    $(".error-message").append("<p>Opps!!! Your tweet is too long!</p>");
    $(".error-message").slideDown("slow");
    $('textarea').focus();
  } else if (violation === 'empty') {
    $(".error-message").hide();
    $(".error-message").empty();
    $(".error-message").append("<p>Please type your tweet...</p>");
    $(".error-message").slideDown("slow");
    $('textarea').focus();
  } else {
    $(".error-message").hide();
...
$(document).ready(function() {
  ...
  $('.submit-and-display button').click(function(event) {
    ...
    const dataLength = loadTweets(data).length;
